...
function generateProductionTask() {
  const tasks = []

  packages.forEach((item) => {
    tasks.push(
      buildScript(`${item.src}/src/**/*.ts`, path.join(item.src, DIST_DIR)),
    )
  })
...
function buildScript(src, dest) {
  return function () {
    return gulp.src(src).pipe(babel(babelConfig)).pipe(gulp.dest(dest))
  }
}
...
exports.build = gulp.series(...generateProductionTask())
exports.watch = gulp.series(...generateBuildTask(), watch)
